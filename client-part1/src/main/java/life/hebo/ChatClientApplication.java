...
public class ChatClientApplication {
    public static void main(String[] args) throws Exception {
        ...
        double estimatedRttMs = 0.5;
        LittlesLawAnalysis.printAnalysis(
                ClientConfig.MAIN_PHASE_THREADS, estimatedRttMs, 0);
        ...
        // Refine RTT estimate from warmup data
        int warmupSuccess = metrics.getSuccessCount().get();
        if (warmupSuccess > 0) {
            // Use warmup throughput to back-calculate effective RTT
            double warmupThroughput = warmupSuccess / (warmupMs / 1000.0);
            estimatedRttMs = ClientConfig.WARMUP_THREADS / warmupThroughput * 1000.0;
        }
        ...
        LittlesLawAnalysis.printAnalysis(
                ClientConfig.MAIN_PHASE_THREADS, estimatedRttMs, actualThroughput);
